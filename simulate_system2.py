# -*- coding: utf-8 -*-
"""
Created on Wed Sep 29 11:17:50 2021
...
def simulate_system2(path, latitude, longitude,
                                     racking,
                                     surface_tilt, 
                                     surface_azimuth,
                                     axis_tilt, 
    ...
    data['fecha'] = pd.to_datetime(data['time'],  format = '%d/%m/%Y %H:%M' )
    
    weather = pd.DataFrame()
    weather['year'] = data.year
    weather['month'] = data.month
    weather['day'] = data.day
    weather['hour'] = data.hour
    weather['minute'] = data.minute
    weather['dni'] = data['DNI']
    weather['ghi'] = data['GHI']
    weather['dhi'] = data['DIF']
